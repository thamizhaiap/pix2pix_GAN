...
from gen_selu import Generator
from disc import Disc
# from disc_selu import Disc
...
def main():
    disc = Disc(in_ch=3).to(config.DEVICE)
    gen = Generator(in_ch= 3, features=64).to(config.DEVICE)
    ...
    val_dataset = MapDataset(root_dir=config.TEST_DIR)
    val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False)

    for epoch in range(config.NUM_EPOCHS):
        train_fn(
            disc, gen, train_loader, opt_disc, opt_gen, L1LOSS, BCE, g_scaler, d_scaler)
