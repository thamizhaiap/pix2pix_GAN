...
class Block(nn.Module):
    def __init__(self, in_ch, out_ch, down=True, act='relu', use_dropout=False):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=4, stride=2, padding=1, bias=False, padding_mode='reflect') 
            if down 
            else nn.ConvTranspose2d(in_ch, out_ch, kernel_size=4, stride =2, padding=1, bias = False),
...
class Generator(nn.Module):
    def __init__(self, in_ch = 3, features = 64):
        ...
        self.initial_down = nn.Sequential(
            nn.Conv2d(in_ch, features, kernel_size=4, stride=2, padding=1, padding_mode='reflect'),
            nn.LeakyReLU(0.2)
        )
...
def test():
    ...
    model = Generator(in_ch=3, features=64)
    preds = model(x)
    print(preds.shape)
